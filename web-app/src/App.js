...
function AuthScreen({onLogin,dark,apiStatus="checking"}) {
  ...
  const verifyOtp=async()=>{
    ...
    try{
      ...
      api.token=res.token; onLogin(res.user,res.token,role);
    }catch{
      onLogin({
    ...
    <div className={`min-h-screen flex flex-col ${t.bg}`}>
      ...
      <div style={{background:"linear-gradient(135deg,#14532d,#16a34a,#22c55e)",paddingTop:64,paddingBottom:48,paddingLeft:24,paddingRight:24,textAlign:"center",color:"#fff"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:8,marginTop:14}}>
          {["70% Owners","10% Drivers","Savings 8%","Loans","Insurance","Pay Later","🌍 Intl"].map(f=>(
...
function DriverApp({user,onLogout,dark,setDark}) {
  ...
  const Nav=()=>(
    <div className={`fixed bottom-0 inset-x-0 max-w-md mx-auto ${t.card} border-t ${t.bdr}`} style={{display:"flex",justifyContent:"space-around",padding:"6px 0",zIndex:30}}>
      {[["home","🏠","Home"],["fintech","💎","Fintech"],["earnings","💰","Earnings"],["profile","👤","Profile"]].map(([v,ic,lb])=>(
          ...
          <div style={{padding:16,display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div className={`${t.card} rounded-2xl p-4 border-2 border-yellow-500`}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                ...
                <span style={{fontWeight:900,color:"#ca8a04",fontSize:20}}>GH₵{(earnings.total*0.03).toFixed(2)}</span>
              </div>
              <p className={`text-xs ${t.sub} mb-3`}>3% of every ride. Use at registered stations.</p>
            ...
            <div className={`${t.card} rounded-2xl p-4 border-2 border-orange-500`}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                ...
                <span style={{fontWeight:900,color:"#ea580c",fontSize:20}}>GH₵{(earnings.total*0.02).toFixed(2)}</span>
              </div>
              <p className={`text-xs ${t.sub}`}>2% auto-collected. Owner approves mechanic payments.</p>
            </div>
          </div>
        )}
...
function OwnerApp({user,onLogout,dark,setDark}) {
  ...
  const Nav=()=>(
    <div className={`fixed bottom-0 inset-x-0 max-w-md mx-auto ${t.card} border-t ${t.bdr}`} style={{display:"flex",justifyContent:"space-around",padding:"6px 0",zIndex:30}}>
      {[["dashboard","📊","Dashboard"],["fintech","💎","Fintech"],["fleet","🚗","Fleet"],["pools","⛽","Pools"]].map(([v,ic,lb])=>(
        ...
        {view==="dashboard"&&(
          <div style={{padding:16,display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div className={`${t.card} rounded-2xl p-4 border ${t.bdr}`}>
              ...
              {[["Your share (70%)","GH₵"+(stats.totalRevenue*0.70).toFixed(0),"#16a34a"],["Driver earnings (10%)","GH₵"+(stats.totalRevenue*0.10).toFixed(0),"#2563eb"],["Fuel pool (3%)","GH₵"+(stats.totalRevenue*0.03).toFixed(0),"#ca8a04"],["Maintenance (2%)","GH₵"+(stats.totalRevenue*0.02).toFixed(0),"#ea580c"],["Platform (15%)","GH₵"+(stats.totalRevenue*0.15).toFixed(0),"#9ca3af"]].map(([l,v,c])=>(
