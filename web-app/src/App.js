...
import { MapPin, Navigation, Star, X, Moon, Sun,
         ...
         Building2, Fuel, Wrench, Eye, EyeOff,
         Copy, Download, Shield, CreditCard, Clock, ArrowDownCircle } from "lucide-react";
...
class Api {
  ...
  verifyGhanaCard(cardNumber,photo)          { return this.req("POST","/verify/ghana-card",{cardNumber,photo}); }
...
const Badge = ({ color, children }) => {
  const c = { green:"bg-green-100 text-green-700", red:"bg-red-50 text-red-500", yellow:"bg-yellow-400 text-gray-900",
              blue:"bg-blue-50 text-blue-600", gray:"bg-gray-100 text-gray-600", purple:"bg-purple-600 text-white", orange:"bg-orange-500 text-white" };
...
function GhanaCardVerify({ role, onVerified, dark }) {
  const t = T(dark);
  ...
  const [step, setStep]         = useState("card"); // card | photo | selfie | review | processing | done
  ...
  const submit = async () => {
    if(!cardNum.match(/^GHA-\d{9}-\d$/i) && cardNum.length < 10) {
      toast$("Enter a valid Ghana Card number (GHA-XXXXXXXXX-X)","error"); return;
    }
    setLoading(true); setStep("processing");
    try { await api.verifyGhanaCard(cardNum, photoFile); } catch {}
    // Simulate NIA verification (manual review in background for now)
    setTimeout(() => {
      setStep("done");
      setLoading(false);
  ...
  return (
    <div className={`min-h-screen ${t.bg} flex flex-col`}>
      ...
      {/* Header */}
      <div style={{background:"linear-gradient(135deg,#1d4ed8,#2563eb)",color:"#fff",padding:"20px 20px 28px",textAlign:"center"}}>
        <div style={{fontSize:40,marginBottom:8}}>🪪</div>
        <h1 style={{fontFamily:"Syne,sans-serif",fontWeight:900,fontSize:22}}>Ghana Card Verification</h1>
        <p style={{color:"#bfdbfe",fontSize:12,marginTop:4}}>Required for all {role}s · Powered by NIA Ghana</p>
        {/* Progress dots */}
        <div style={{display:"flex",justifyContent:"center",gap:8,marginTop:16}}>
          {["card","photo","selfie","review"].map((s,i)=>(
            <div key={s} style={{width:8,height:8,borderRadius:"50%",background:["card","photo","selfie","review","processing","done"].indexOf(step)>=i?"#fff":"rgba(255,255,255,0.3)"}}/>
          ))}
        </div>
      </div>
      ...
      <div style={{flex:1,padding:"24px 20px",display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div className={`${t.card} rounded-2xl p-5 border ${t.bdr}`}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
              <div style={{width:36,height:36,borderRadius:"50%",background:"#dbeafe",display:"flex",alignItems:"center",justifyContent:"center"}}>
                <span style={{fontWeight:900,color:"#2563eb"}}>1</span>
              </div>
              <div>
                <p className={`font-black ${t.text}`}>Enter Ghana Card Number</p>
                <p className={`text-xs ${t.sub}`}>Format: GHA-XXXXXXXXX-X</p>
              </div>
            </div>
            <input
              value={cardNum}
              ...
              maxLength={15}
              className={`w-full px-4 py-4 border rounded-2xl text-lg text-center font-black font-mono focus:outline-none focus:ring-2 focus:ring-blue-500 ${t.inp}`}
              style={{display:"block",width:"100%",letterSpacing:"0.05em"}}
            />
            <p className={`text-xs mt-3 ${t.sub}`} style={{textAlign:"center"}}>
              Found on the front of your Ghana National ID card
            </p>
            <button
              onClick={()=>{ if(cardNum.length < 8){toast$("Enter Ghana Card number","error");return;} setStep("photo"); }}
              style={{width:"100%",marginTop:16,padding:"14px",background:"#2563eb",color:"#fff",borderRadius:16,fontWeight:900,fontSize:15}}>
              Next — Upload Card Photo →
            </button>
          </div>
        )}
        ...
        {step==="photo" && (
          <div className={`${t.card} rounded-2xl p-5 border ${t.bdr}`}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
              <div style={{width:36,height:36,borderRadius:"50%",background:"#dbeafe",display:"flex",alignItems:"center",justifyContent:"center"}}>
                <span style={{fontWeight:900,color:"#2563eb"}}>2</span>
              </div>
              <div>
                <p className={`font-black ${t.text}`}>Photo of Your Ghana Card</p>
                <p className={`text-xs ${t.sub}`}>Front side, clear & well-lit</p>
              </div>
            </div>
            {photoPreview ? (
              <div style={{position:"relative",marginBottom:12}}>
                <img src={photoPreview} alt="Card" style={{width:"100%",borderRadius:12,maxHeight:200,objectFit:"cover"}}/>
                <button onClick={()=>{setPhotoPreview(null);setPhotoFile(null);}} style={{position:"absolute",top:8,right:8,background:"#dc2626",color:"#fff",borderRadius:"50%",width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center"}}>
                  <X style={{width:14,height:14}}/>
                </button>
              </div>
            ) : (
              <label style={{display:"block",border:"2px dashed #93c5fd",borderRadius:16,padding:"32px 16px",textAlign:"center",cursor:"pointer",background:dark?"#1e3a5f":"#eff6ff",marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setStep("card")} style={{flex:1,padding:"12px",border:`1px solid ${dark?"#374151":"#e5e7eb"}`,borderRadius:14,fontWeight:700,fontSize:13}} className={t.text}>← Back</button>
              {photoPreview && <button onClick={()=>setStep("selfie")} style={{flex:2,padding:"12px",background:"#2563eb",color:"#fff",borderRadius:14,fontWeight:900,fontSize:13}}>Next — Take Selfie →</button>}
            </div>
          </div>
        )}
        ...
        {step==="selfie" && (
          <div className={`${t.card} rounded-2xl p-5 border ${t.bdr}`}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
              <div style={{width:36,height:36,borderRadius:"50%",background:"#dbeafe",display:"flex",alignItems:"center",justifyContent:"center"}}>
                <span style={{fontWeight:900,color:"#2563eb"}}>3</span>
              </div>
              <div>
                <p className={`font-black ${t.text}`}>Take a Selfie</p>
                <p className={`text-xs ${t.sub}`}>Face must match your Ghana Card photo</p>
              </div>
            </div>
            {selfiePreview ? (
              <div style={{position:"relative",marginBottom:12}}>
                <img src={selfiePreview} alt="Selfie" style={{width:"100%",borderRadius:12,maxHeight:200,objectFit:"cover"}}/>
                <button onClick={()=>{setSelfiePreview(null);setSelfie(null);}} style={{position:"absolute",top:8,right:8,background:"#dc2626",color:"#fff",borderRadius:"50%",width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center"}}>
                  <X style={{width:14,height:14}}/>
                </button>
              </div>
            ) : (
              <label style={{display:"block",border:"2px dashed #86efac",borderRadius:16,padding:"32px 16px",textAlign:"center",cursor:"pointer",background:dark?"#14532d":"#f0fdf4",marginBottom:12}}>
                ...
                <p className={`text-xs ${t.sub} mt-1`}>Use front camera · Face clearly visible</p>
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              {selfiePreview && <button onClick={()=>setStep("review")} style={{flex:2,padding:"12px",background:"#2563eb",color:"#fff",borderRadius:14,fontWeight:900,fontSize:13}}>Review →</button>}
            </div>
          </div>
        )}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div className={`${t.card} rounded-2xl p-5 border ${t.bdr}`}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 12px",borderRadius:12,background:dark?"#374151":"#f9fafb"}}>
                  <span className={`text-sm ${t.sub}`}>Ghana Card Number</span>
                  <span style={{fontFamily:"monospace",fontWeight:900,color:"#2563eb",fontSize:13}}>{cardNum}</span>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  {photoPreview && <div style={{textAlign:"center"}}>
                    <p className={`text-xs ${t.sub} mb-1`}>Card Photo</p>
                    <img src={photoPreview} alt="card" style={{width:"100%",borderRadius:8,height:80,objectFit:"cover"}}/>
                  </div>}
                  {selfiePreview && <div style={{textAlign:"center"}}>
                    <p className={`text-xs ${t.sub} mb-1`}>Selfie</p>
                    <img src={selfiePreview} alt="selfie" style={{width:"100%",borderRadius:8,height:80,objectFit:"cover"}}/>
                  </div>}
                </div>
              </div>
            </div>
            <div className={`${t.card} rounded-2xl p-4 border border-blue-200`} style={{background:dark?"#1e3a5f":"#eff6ff"}}>
              <p style={{color:"#2563eb",fontWeight:700,fontSize:12,marginBottom:4}}>🔒 Privacy Notice</p>
              <p className={`text-xs ${t.sub}`}>Your Ghana Card data is encrypted and used solely for identity verification. It will not be shared with third parties. Data handled in compliance with Ghana's Data Protection Act 2012.</p>
            </div>
        ...
        {step==="processing" && (
          <div style={{textAlign:"center",padding:"48px 0"}}>
            <div style={{width:64,height:64,border:"4px solid #2563eb",borderTopColor:"transparent",borderRadius:"50%",animation:"spin 1s linear infinite",margin:"0 auto 20px"}}/>
            <p className={`font-black text-lg ${t.text}`}>Verifying Identity…</p>
            <p className={`text-xs mt-2 ${t.sub}`}>Checking with NIA Ghana database</p>
            <p className={`text-xs mt-1 ${t.sub}`}>This takes a few seconds</p>
          </div>
        )}
        ...
        {step==="done" && (
          <div style={{textAlign:"center",padding:"48px 0"}}>
            ...
            <p className={`text-sm mt-2 ${t.sub}`}>Your account is under review.</p>
            <p className={`text-xs mt-1 ${t.sub}`}>Manual review takes up to 24 hours. You'll receive an SMS once approved.</p>
          </div>
        )}
        ...
        {(step==="card"||step==="photo") && (
          <div className={`${t.card} rounded-2xl p-4 border ${t.bdr}`}>
            <p className={`font-bold text-sm mb-2 ${t.text}`}>🪪 Why Ghana Card?</p>
            {["Accurate legal name & date of birth automatically","No manual data entry — faster onboarding","Required for Drive to Own vehicle financing","Enables insurance & bank integrations","Protects passengers with verified driver identities"].map(r=>(
...
function WithdrawSheet({ available, pending, userId, onClose, dark }) {
  ...
  const withdraw = async () => {
    if(!amount || parseFloat(amount) < 1) { toast$("Enter a valid amount","error"); return; }
    if(parseFloat(amount) > available) { toast$("Amount exceeds available balance","error"); return; }
      ...
      <div className={`${t.card} rounded-t-3xl p-6 w-full shadow-2xl`} style={{maxHeight:"90vh",overflowY:"auto"}}>
        ...
        {step==="form" && (<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
            <div style={{background:"#f0fdf4",borderRadius:14,padding:"12px",textAlign:"center"}}>
              ...
              <p style={{fontWeight:900,color:"#16a34a",fontSize:22}}>GH₵{available.toFixed(2)}</p>
              <p style={{fontSize:10,color:"#16a34a"}}>Ready to withdraw</p>
            </div>
            <div style={{background:dark?"#374151":"#f9fafb",borderRadius:14,padding:"12px",textAlign:"center"}}>
              <p style={{fontSize:11,color:"#ca8a04",fontWeight:700}}>PENDING</p>
              <p style={{fontWeight:900,color:"#ca8a04",fontSize:22}}>GH₵{pending.toFixed(2)}</p>
              <p style={{fontSize:10,color:"#ca8a04"}}>Unlocks in 24hrs</p>
            </div>
          </div>

          {/* 24hr notice */}
          <div style={{background:dark?"#1c1917":"#fefce8",borderRadius:12,padding:"10px 12px",marginBottom:16,display:"flex",gap:8,alignItems:"flex-start"}}>
            <Clock style={{width:14,height:14,color:"#ca8a04",flexShrink:0,marginTop:1}}/>
            <p style={{fontSize:11,color:"#92400e"}}>Earnings are held for <strong>24 hours</strong> before withdrawal. This protects against fraudulent rides and ensures all payments are verified.</p>
          </div>
          ...
          <input
            ...
            className={`w-full px-4 py-3 border rounded-xl text-lg font-black focus:outline-none focus:ring-2 focus:ring-green-500 ${t.inp}`}
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
            {[["mtn","MTN","#ffcc00","#000"],["vodafone","Vodafone","#e60000","#fff"],["airtel","AirtelTigo","#ef4444","#fff"]].map(([id,label,bg,fg])=>(
          ...
          <input
            value={momoPhone} onChange={e=>setMomoPhone(e.target.value)} type="tel"
            placeholder="+233XXXXXXXXX"
            className={`w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500 ${t.inp}`}
          ...
          <button onClick={withdraw} disabled={!amount||!momoPhone||parseFloat(amount)>available}
            style={{width:"100%",padding:"14px",background:(!amount||!momoPhone||parseFloat(amount)>available)?"#9ca3af":"#16a34a",color:"#fff",borderRadius:16,fontWeight:900,fontSize:15,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
            <ArrowDownCircle style={{width:18,height:18}}/> Withdraw GH₵{amount||"0.00"}
          </button>
          <p className={`text-xs text-center mt-2 ${t.sub}`}>Funds arrive on MoMo within 5 minutes</p>
        </>)}
        ...
        {step==="processing" && (
          <div style={{textAlign:"center",padding:"32px 0"}}>
            ...
            <p className={`font-black ${t.text}`}>Processing Withdrawal…</p>
            <p className={`text-xs mt-1 ${t.sub}`}>Sending GH₵{amount} via {network.toUpperCase()} MoMo</p>
          </div>
        )}
        ...
        {step==="done" && (
          <div style={{textAlign:"center",padding:"32px 0"}}>
            ...
            <p className={`text-xs mt-1 ${t.sub}`}>Arrives within 5 minutes</p>
...
function AuthScreen({ onLogin, dark, apiStatus="checking" }) {
  ...
  const [ghanaVerified, setGhanaVerified] = useState(null);
  const [loading,setLoading] = useState(false);
  const [toast,setToast] = useState(null);
  const toast$ = (msg,type="success") => setToast({msg,type});

  const needsGhanaCard = role === "driver" || role === "owner";
  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      const demo = {
        ...
        ghanaCard: ghanaVerified?.ghanaCard || null,
  ...
  // Show Ghana Card verification for drivers/owners after OTP step
  if(step==="ghana-card") {
    return <GhanaCardVerify role={role} dark={dark} onVerified={(data)=>{ setGhanaVerified(data); setStep("phone"); toast$("Ghana Card submitted ✅ Proceed to get OTP"); }}/>;
  }
    ...
    <div className={`min-h-screen flex flex-col ${t.bg}`}>
      ...
      <div style={{background:"linear-gradient(135deg,#14532d,#16a34a,#22c55e)",paddingTop:64,paddingBottom:48,paddingLeft:24,paddingRight:24,textAlign:"center",color:"#fff"}}>
        ...
        <div style={{marginTop:8,display:"inline-flex",alignItems:"center",gap:6,background:"rgba(0,0,0,0.2)",borderRadius:999,padding:"4px 12px"}}>
          <div style={{width:8,height:8,borderRadius:"50%",background:apiStatus==="ok"?"#4ade80":apiStatus==="error"?"#f87171":"#facc15",animation:apiStatus==="checking"?"pulse 1s infinite":"none"}}/>
          <span style={{fontSize:11,fontWeight:700,color:"#fff"}}>{apiStatus==="ok"?"Backend Connected ✅":apiStatus==="error"?"Backend Offline ❌":"Connecting…"}</span>
        </div>
      ...
      <div style={{flex:1,padding:"24px 20px"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",borderRadius:16,overflow:"hidden",border:`1px solid ${dark?"#374151":"#e5e7eb"}`,marginBottom:24}}>
          {[["passenger","🧍","Passenger"],["driver","🏍️","Driver"],["owner","🏢","Owner"],["admin","⚙️","Admin"]].map(([r,ic,lb])=>(
            <button key={r} onClick={()=>{setRole(r);setStep("phone");setGhanaVerified(null);}}
        ...
        {step==="phone" && (
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            {/* Ghana Card prompt for drivers/owners */}
            {needsGhanaCard && (
              <div className={`rounded-2xl p-4 border-2 ${ghanaVerified?"border-green-500":"border-blue-400"}`} style={{background:ghanaVerified?(dark?"#14532d":"#f0fdf4"):(dark?"#1e3a5f":"#eff6ff")}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <span style={{fontSize:20}}>🪪</span>
                    <div>
                      <p style={{fontWeight:900,fontSize:13,color:ghanaVerified?"#16a34a":"#2563eb"}}>Ghana Card Verification</p>
                      <p className={`text-xs ${t.sub}`}>{ghanaVerified?`Submitted: ${ghanaVerified.ghanaCard}`:"Required for drivers & owners"}</p>
                    </div>
                  </div>
                  {ghanaVerified
                    ? <CheckCircle style={{width:20,height:20,color:"#16a34a"}}/>
                    : <button onClick={()=>setStep("ghana-card")} style={{padding:"6px 12px",background:"#2563eb",color:"#fff",borderRadius:10,fontWeight:700,fontSize:12}}>Verify →</button>
                  }
                </div>
              </div>
            )}
        ...
        <div className="grid grid-cols-2 gap-3 mt-8">
          {[["🪪","Ghana Card KYC"],["💰","70/10/3/2/15 splits"],["⛽","Auto fuel pool"],["🔧","Auto maintenance"],["📱","USSD *711# support"],["🔒","24hr withdrawal"]].map(([i,l])=>(
...
function PassengerApp({ user, onLogout, dark, setDark }) {
  ...
  const [loading,setLoading] = useState(false);
  const [toast,setToast]     = useState(null);
  const [payStatus,setPayStatus] = useState("idle"); // idle|selecting|processing|paid
  ...
  useEffect(()=>{
    if(view==="history"){
      api.getHistory(user.id).then(r=>setHistory(r.rides||[])).catch(()=>setHistory(
        Array.from({length:6},(_,i)=>({id:i,from:LOCS[i*2%LOCS.length],to:LOCS[(i*2+1)%LOCS.length],fare:(Math.random()*20+5).toFixed(2),date:new Date(Date.now()-i*86400000*2).toLocaleDateString(),driver:"Kwame A.",rating:5,vehicle:VEHICLES[i%4].label,paid:"MoMo"}))
      ));
    }
  },[view,user.id]);
              ...
              {status==="ongoing"&&driver&&(
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    ...
                    <button onClick={()=>setPayStatus("selecting")} style={{padding:"12px",background:"#16a34a",color:"#fff",borderRadius:12,fontWeight:700,fontSize:13}}>Pay & Complete 💳</button>
                  </div>
                </div>
              )}
        ...
        {payStatus==="selecting"&&fare&&(
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:60,display:"flex",alignItems:"flex-end",maxWidth:448,margin:"0 auto"}}>
            <div className={`${t.card} rounded-t-3xl p-6 w-full shadow-2xl`} style={{maxHeight:"90vh",overflowY:"auto"}}>
              ...
              <p className={`text-xs ${t.sub} mb-4`}>{pickup} → {dest}</p>
              <div style={{background:dark?"#14532d":"#f0fdf4",borderRadius:12,padding:12,marginBottom:16,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span className={`font-semibold ${t.text}`}>Total Amount</span>
                <span style={{fontWeight:900,color:"#16a34a",fontSize:22}}>GH₵{fare.total}</span>
              </div>

              <p className={`text-xs font-bold mb-2 ${t.sub}`}>SELECT PAYMENT METHOD</p>
              ...
              {payMethod==="card"&&(
                <div style={{marginBottom:12}}>
                  <p className={`text-xs font-bold mb-1 ${t.sub}`}>Email for receipt</p>
              ...
              {payMethod==="cash"&&(
                <div style={{background:dark?"#374151":"#f9fafb",borderRadius:12,padding:12,marginBottom:12}}>
                  ...
                  <p className={`text-xs ${t.sub}`}>Hand GH₵{fare.total} cash to your driver. The driver must confirm receipt on their app before the ride closes.</p>
                  <div style={{marginTop:8,padding:"8px",background:dark?"#1f2937":"#fef3c7",borderRadius:8,display:"flex",gap:6,alignItems:"flex-start"}}>
                    <AlertCircle style={{width:12,height:12,color:"#ca8a04",flexShrink:0,marginTop:1}}/>
                    <p style={{fontSize:11,color:"#92400e"}}>Driver confirmation required — ride won't close until they tap confirm on their app.</p>
                  </div>
                </div>
              )}
              ...
              <button
                ...
                onClick={async()=>{
                  if(!payMethod) return;
                  if(payMethod==="cash"){
                    // Show waiting state — driver must confirm
                    setPayStatus("awaiting-driver");
                    toast$("Waiting for driver to confirm cash payment… 💵");
                    // Simulate driver confirming after 4 seconds
                    setTimeout(()=>{
                      setPayStatus("paid");
                      setStatus("idle");setDriver(null);setPickup("");setDest("");setFare(null);
                      toast$("Cash confirmed by driver! Ride complete 🇬🇭");
                  ...
                  // Paystack processes → backend webhook fires → ride marked paid
                  setTimeout(()=>{
                    setPayStatus("paid");
                    setStatus("idle");setDriver(null);setPickup("");setDest("");setFare(null);
                    toast$(`Payment verified! GH₵${fare.total} paid via ${payMethod.toUpperCase()} 🎉`);
                    setTimeout(()=>setPayStatus("idle"),3000);
                  },2500);
                }}
                style={{width:"100%",padding:"14px",background:!payMethod?"#9ca3af":"#16a34a",color:"#fff",borderRadius:16,fontWeight:900,fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",gap:8,opacity:!payMethod?0.5:1}}>
                {payStatus==="processing"?<><Spin/>Processing…</>:payMethod==="cash"?`Notify Driver — GH₵${fare.total} Cash`:`Pay GH₵${fare.total} via Paystack 🔒`}
              </button>
              <p className={`text-xs text-center mt-3 ${t.sub}`}>🔒 Secured by Paystack · MTN · Vodafone · Airtel · Card</p>
            </div>
          </div>
        )}

        {/* Awaiting driver cash confirmation */}
        {payStatus==="awaiting-driver"&&(
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:60,display:"flex",alignItems:"center",justifyContent:"center",maxWidth:448,margin:"0 auto"}}>
            <div className={`${t.card} rounded-3xl p-8 mx-4 shadow-2xl`} style={{textAlign:"center"}}>
              ...
              <p className={`text-sm mt-2 ${t.sub}`}>Driver must confirm they received GH₵{fare?.total} cash</p>
              <p className={`text-xs mt-3 ${t.sub}`}>This prevents payment disputes</p>
            </div>
          </div>
        )}
              ...
              <div key={r.id} className={`${t.card} rounded-2xl p-4 mb-3 border ${t.bdr}`}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <div style={{flex:1,gap:4,display:"flex",flexDirection:"column"}}>
                    ...
                    <Badge color="green">{r.paid||"MoMo"} ✓</Badge>
                  </div>
...
function DriverApp({ user, onLogout, dark, setDark }) {
  ...
  const [cashConfirm,setCashConfirm] = useState(null); // ride needing cash confirmation
  ...
  const [toast,setToast]       = useState(null);
  const [showGhanaCard,setShowGhanaCard] = useState(!user.ghanaCard);
  const toast$ = (msg,type="success")=>setToast({msg,type});

  useEffect(()=>{
    if(!online||incoming||activeRide||cashConfirm)return;
    const tm=setTimeout(()=>setIncoming({
      id:"ride_"+Date.now(),passenger:"Ama Owusu",phone:"+233205556789",
      from:"Akosombo",to:"Atimpoku",dist:"4.2 km",dur:"12 min",
      fare:"GH₵13.50",earn:"GH₵1.35",payMethod:"mtn"
    }),5000);
  ...
  // Simulate cash payment request coming from passenger
  useEffect(()=>{
    if(!activeRide)return;
    if(activeRide.payMethod==="cash"){
      const tm=setTimeout(()=>{
        setCashConfirm(activeRide);
      },8000);
      return()=>clearTimeout(tm);
    }
  },[activeRide]);
  ...
  const confirmCash=async()=>{
    ...
    // Earnings go to pending for 24hrs, then available
    setWallet(w=>({available:w.available,pending:+(w.pending+earned).toFixed(2)}));
    setTimeout(()=>setWallet(w=>({available:+(w.available+earned).toFixed(2),pending:+(w.pending-earned).toFixed(2)})),5000); // demo: 5s instead of 24hrs
  ...
  const complete=()=>{
    ...
    setActiveRide(null);
    toast$(`Ride complete! GH₵${earned.toFixed(2)} earned — pending 24hr hold 💰`);
  };
  ...
  // Ghana Card verification gate
  if(showGhanaCard) {
    return <GhanaCardVerify role="driver" dark={dark} onVerified={()=>setShowGhanaCard(false)}/>;
  }
    ...
    <div className={`max-w-md mx-auto min-h-screen ${t.bg}`}>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:60,display:"flex",alignItems:"center",justifyContent:"center",maxWidth:448,margin:"0 auto",padding:"0 20px"}}>
          <div className={`${t.card} rounded-3xl p-6 w-full shadow-2xl`}>
            <div style={{textAlign:"center",marginBottom:16}}>
              ...
              <h3 className={`font-black text-lg ${t.text}`}>Cash Payment Confirmation</h3>
              <p className={`text-sm ${t.sub} mt-1`}>{cashConfirm.passenger} says they paid you cash</p>
            </div>
            <div style={{background:dark?"#374151":"#f0fdf4",borderRadius:14,padding:"16px",textAlign:"center",marginBottom:16}}>
              <p className={`text-xs ${t.sub}`}>Amount to confirm</p>
              <p style={{fontWeight:900,color:"#16a34a",fontSize:32}}>GH₵{cashConfirm.fare?.replace("GH₵","")}</p>
              <p className={`text-xs ${t.sub} mt-1`}>{cashConfirm.from} → {cashConfirm.to}</p>
            </div>
            <div style={{background:dark?"#1c1917":"#fef3c7",borderRadius:12,padding:"10px 12px",marginBottom:16,display:"flex",gap:8}}>
              <AlertCircle style={{width:14,height:14,color:"#ca8a04",flexShrink:0,marginTop:1}}/>
              <p style={{fontSize:11,color:"#92400e"}}>Only confirm if you have physically received the cash. This action cannot be undone and will close the ride.</p>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <button onClick={()=>{setCashConfirm(null);toast$("Cash dispute raised — admin will review","error");}}
                style={{padding:"12px",border:"1px solid #f87171",color:"#ef4444",borderRadius:14,fontWeight:700,fontSize:13}}>
                ❌ Not Received
              </button>
              <button onClick={confirmCash}
                style={{padding:"12px",background:"#16a34a",color:"#fff",borderRadius:14,fontWeight:900,fontSize:13}}>
                ✅ Confirm Cash
              </button>
            </div>
          </div>
      ...
      <div style={{background:"#166534",color:"#fff",padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:20}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,padding:"6px 12px",borderRadius:999,background:online?"#16a34a":"#4b5563",fontSize:11,fontWeight:900}}>
          <div style={{width:7,height:7,borderRadius:"50%",background:online?"#fff":"#9ca3af"}}/>
          ...
          <div style={{padding:16,display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div className={`${t.card} rounded-2xl p-4 border-2 border-green-500`}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                ...
                <button onClick={()=>setShowWithdraw(true)} style={{padding:"6px 14px",background:"#16a34a",color:"#fff",borderRadius:10,fontWeight:700,fontSize:12,display:"flex",alignItems:"center",gap:4}}>
            ...
            <div className={`${t.card} rounded-2xl p-4 border-2 border-yellow-500`}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                ...
                <span style={{fontSize:22,fontWeight:900,color:"#ca8a04"}}>GH₵{(earnings.total*0.03).toFixed(2)}</span>
              </div>
              <p className={`text-xs ${t.sub} mb-3`}>3% of every ride auto-collected. Use only at registered fuel stations.</p>
              ...
              {showFuelCode&&(
                <div style={{marginTop:12,textAlign:"center",padding:"14px",background:dark?"#374151":"#fefce8",borderRadius:12,border:"2px dashed #ca8a04"}}>
                  <p className={`text-xs ${t.sub} mb-2`}>Your fuel station code</p>
                  <p style={{fontFamily:"monospace",fontSize:26,fontWeight:900,color:"#ca8a04",letterSpacing:"0.15em"}}>{fuelCode}</p>
                  <p className={`text-xs ${t.sub} mt-2`}>Valid 10 minutes · Balance: GH₵{(earnings.total*0.03).toFixed(2)}</p>
                </div>
              )}
            </div>
            ...
            <div className={`${t.card} rounded-2xl p-4 border-2 border-orange-500`}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                ...
                <span style={{fontSize:22,fontWeight:900,color:"#ea580c"}}>GH₵{(earnings.total*0.02).toFixed(2)}</span>
              </div>
              <p className={`text-xs ${t.sub}`}>2% auto-collected. Owner approves mechanic payments. No more "vehicle broken, no money" problems.</p>
            </div>
        ...
        {view==="earnings"&&(
          <div style={{padding:16,display:"flex",flexDirection:"column",gap:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <button onClick={()=>setShowWithdraw(true)} style={{padding:"8px 14px",background:"#16a34a",color:"#fff",borderRadius:12,fontWeight:700,fontSize:12,display:"flex",alignItems:"center",gap:4}}>
                <ArrowDownCircle style={{width:12,height:12}}/> Withdraw
              </button>
            </div>
            <div className={`${t.card} rounded-2xl p-5 border ${t.bdr}`} style={{textAlign:"center"}}>
              <p className={`text-sm ${t.sub}`}>Total Lifetime (10% share)</p>
              <p style={{fontSize:40,fontWeight:900,color:"#16a34a",margin:"4px 0"}}>GH₵{earnings.total}</p>
              <p className={`text-xs ${t.sub}`}>Transparent · Automatic · Fair</p>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <div className={`${t.card} rounded-2xl p-4 border-2 border-green-500`} style={{textAlign:"center"}}>
                <p style={{fontSize:10,color:"#16a34a",fontWeight:700,marginBottom:4}}>AVAILABLE NOW</p>
                <p style={{fontWeight:900,color:"#16a34a",fontSize:24}}>GH₵{wallet.available.toFixed(2)}</p>
                <p style={{fontSize:10,color:"#16a34a"}}>Ready to withdraw</p>
              </div>
              <div className={`${t.card} rounded-2xl p-4 border-2 border-yellow-400`} style={{textAlign:"center"}}>
                <p style={{fontSize:10,color:"#ca8a04",fontWeight:700,marginBottom:4}}>PENDING</p>
                <p style={{fontWeight:900,color:"#ca8a04",fontSize:24}}>GH₵{wallet.pending.toFixed(2)}</p>
                <p style={{fontSize:10,color:"#ca8a04"}}>Unlocks in 24hrs</p>
              </div>
            </div>
            <div className={`${t.card} rounded-2xl p-4 border ${t.bdr}`} style={{display:"flex",gap:8,alignItems:"flex-start"}}>
              <Clock style={{width:16,height:16,color:"#ca8a04",flexShrink:0,marginTop:2}}/>
              <div>
                <p className={`font-bold text-sm ${t.text}`}>24-Hour Hold Policy</p>
                <p className={`text-xs ${t.sub} mt-1`}>All earnings are held for 24 hours before withdrawal. This protects against fraudulent rides, ensures Paystack payments have cleared, and allows dispute resolution. Your money is always safe.</p>
              </div>
            </div>
...
function OwnerApp({ user, onLogout, dark, setDark }) {
  ...
  if(showGhanaCard) {
    return <GhanaCardVerify role="owner" dark={dark} onVerified={()=>setShowGhanaCard(false)}/>;
  }
          ...
          <div style={{padding:16,display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div className={`${t.card} rounded-2xl p-4 border-2 border-blue-500`}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                ...
                <button onClick={()=>setShowWithdraw(true)} style={{padding:"6px 14px",background:"#2563eb",color:"#fff",borderRadius:10,fontWeight:700,fontSize:12,display:"flex",alignItems:"center",gap:4}}>
                  <ArrowDownCircle style={{width:12,height:12}}/> Withdraw
                </button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            ...
            <div className={`${t.card} rounded-2xl p-4 border-2 border-green-500`}>
              ...
              {["No daily arguments with drivers — splits are automatic","Fuel always available (3% pool auto-fills)","Maintenance never missed (2% pool + alerts)","See ALL trips in real-time","All drivers verified via Ghana Card"].map(b=>(
...
function AdminApp({ user, onLogout, dark, setDark }) {
  ...
  const [stats,setStats]=useState({totalRides:5432,activeRides:23,totalDrivers:87,onlineDrivers:34,revenue:18450,commission:2767,users:1247,owners:42,pendingVerifications:14});
  const [toast,setToast]=useState(null);
  const toast$=(msg,type="success")=>setToast({msg,type});
  ...
  const liveRides=[
    ...
    {id:"R-003",pax:"Abena T.",driver:"Akosua S.",from:"Odumase", to:"Somanya", fare:"₵11.00",status:"matched",  pay:"Card"},
    {id:"R-004",pax:"Kweku B.",driver:"Kofi A.",  from:"Atimpoku",to:"Senchi",  fare:"₵7.00", status:"ongoing",  pay:"MoMo"},
  ];
  const drivers=[
    {name:"Kwame Asante",  phone:"+233241234567",plate:"ER-1234-26",rating:4.9,rides:1247,online:true, earn:4250,card:"GHA-123456789-1"},
    {name:"Yaw Mensah",    phone:"+233209876543",plate:"ER-5678-26",rating:4.8,rides:876, online:true, earn:3890,card:"GHA-987654321-2"},
    {name:"Akosua Sarpong",phone:"+233285556789",plate:"ER-9012-26",rating:4.7,rides:534, online:false,earn:2650,card:"Pending"},
    {name:"Kofi Adjei",    phone:"+233544444444",plate:"ER-3456-26",rating:4.6,rides:289, online:true, earn:1780,card:"Pending"},
  ];
  const withdrawals=[
  ...
  return (
    <div className={`max-w-md mx-auto min-h-screen ${t.bg}`}>
      ...
      <div style={{paddingBottom:24}}>
        {view==="overview"&&(
          <div style={{padding:16,display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              ...
              <StatCard icon="🪪" label="KYC Pending"   value={stats.pendingVerifications||14}     sub="Ghana Card reviews" color="yellow" dark={dark}/>
            ...
            {/* KYC Queue */}
            <div className={`${t.card} rounded-2xl p-4 border-2 border-blue-400`}>
              <p style={{color:"#2563eb",fontWeight:900,fontSize:13,marginBottom:10}}>🪪 Ghana Card Verification Queue</p>
              {[["Kofi Adjei","Driver","2h ago"],["Abena Tetteh","Owner","4h ago"],["Yaw Asare","Driver","6h ago"]].map(([n,r,t2])=>(
                <div key={n} style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingBottom:8,marginBottom:8,borderBottom:`1px solid ${dark?"#374151":"#e5e7eb"}`}}>
                  <div>
                    <p className={`font-bold text-sm ${t.text}`}>{n}</p>
                    <p className={`text-xs ${t.sub}`}>{r} · Submitted {t2}</p>
                  </div>
                  <div style={{display:"flex",gap:6}}>
                    <button onClick={()=>toast$(`${n} approved ✅`)} style={{padding:"6px 10px",background:"#16a34a",color:"#fff",borderRadius:8,fontWeight:700,fontSize:11}}>Approve</button>
                    <button onClick={()=>toast$(`${n} rejected`,"error")} style={{padding:"6px 10px",background:"#fef2f2",color:"#ef4444",borderRadius:8,fontWeight:700,fontSize:11}}>Reject</button>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div key={r.id} className={`${t.card} rounded-2xl p-4 border ${t.bdr} mb-3`}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  ...
                  <div style={{display:"flex",gap:4}}>
                    ...
                    <Badge color={r.pay==="Cash"?"yellow":"blue"}>{r.pay}</Badge>
                  </div>
                </div>
                ...
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:4,marginTop:4,flexWrap:"wrap"}}>
                      ...
                      <Badge color={d.card==="Pending"?"yellow":"blue"}>{d.card==="Pending"?"🪪 KYC Pending":"🪪 "+d.card.slice(0,7)+"…"}</Badge>
                    </div>
                  </div>
        ...
        {view==="finance"&&(
          <div style={{padding:16,display:"flex",flexDirection:"column",gap:16}}>
            <h2 className={`font-black text-lg ${t.text}`}>Finance & Withdrawals</h2>
            ...
            <div className={`${t.card} rounded-2xl border ${t.bdr} overflow-hidden`}>
              <div className={`px-4 py-3 flex items-center gap-2 ${dark?"bg-gray-700":"bg-gray-50"}`}>
                <ArrowDownCircle style={{width:18,height:18,color:"#16a34a"}}/>
                <span className={`font-black ${t.text}`}>Recent Withdrawals</span>
              </div>
              {withdrawals.map(w=>(
                <div key={w.name} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",borderBottom:`1px solid ${dark?"#374151":"#e5e7eb"}`}}>
                  <div>
                    <p className={`font-bold text-sm ${t.text}`}>{w.name}</p>
                    <p className={`text-xs ${t.sub}`}>{w.method} · {w.time}</p>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <p style={{fontWeight:900,color:"#16a34a"}}>GH₵{w.amount}</p>
                    <Badge color={w.status==="completed"?"green":"yellow"}>{w.status}</Badge>
                  </div>
                </div>
              ))}
            </div>
