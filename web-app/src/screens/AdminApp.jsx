        ...
        {view === "overview" && (
          <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div className={`${t.card} rounded-2xl p-4 border ${t.bdr}`}>
              ...
              {[
                ["Total GMV", "₵" + stats.revenue.toLocaleString(), "#16a34a"],
                ["Platform 15%", "₵" + stats.commission.toLocaleString(), "#2563eb"],
                ["Owner payouts 50%", "₵" + (stats.revenue * 0.5).toFixed(0), "#9333ea"],
                ["Driver payouts 25%", "₵" + (stats.revenue * 0.25).toFixed(0), "#ea580c"],
                ["Fuel pools 5%", "₵" + (stats.revenue * 0.05).toFixed(0), "#ca8a04"],
                ["Maintenance 5%", "₵" + (stats.revenue * 0.05).toFixed(0), "#f97316"],
              ].map(([l, v, c]) => (
        ...
        {view === "kyc" && (
          <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div className={`${t.card} rounded-2xl p-4 border ${t.bdr}`}>
              ...
              {[
                ["Total Verified", "1,233 users", "#16a34a"],
                ["Ghana Cards", "1,018", "#2563eb"],
                ["International Passports", "215", "#7c3aed"],
                ["Pending Review", "14", "#ca8a04"],
                ["Rejected (30 days)", "6", "#ef4444"],
              ].map(([l, v, c]) => (
                <div key={l} style={{ display: "flex", justifyContent: "space-between", paddingBottom: 8, borderBottom: `1px solid ${dark ? "#374151" : "#e5e7eb"}`, marginBottom: 8, fontSize: 13 }}>
                  <span className={t.sub}>{l}</span><span style={{ fontWeight: 700, color: c }}>{v}</span>
                </div>
