...
export function FintechHub({ user, role, dark }) {
  ...
  // ── Savings state
  const [savBal, setSavBal] = useState(role === "driver" ? 320.5 : role === "owner" ? 1840.0 : 45.0);
  const [savInt, setSavInt] = useState(role === "driver" ? 12.4 : role === "owner" ? 87.2 : 1.8);
  const [autoRate, setAutoRate] = useState(10);
  const [savStep, setSavStep] = useState("home");
  const [savInput, setSavInput] = useState("");
  const savHistory = [
    { date: "Mar 2026", deposited: 85.2, interest: 2.1, balance: savBal },
    { date: "Feb 2026", deposited: 92.4, interest: 1.9, balance: 233.2 },
    { date: "Jan 2026", deposited: 78.6, interest: 1.6, balance: 139.2 },
  ];
  ...
  const [activeLoan, setActiveLoan] = useState(
    role === "driver"
      ? { amount: 800, remaining: 520, repaid: 280, monthly: 65, purpose: "Okada Repair", progress: 35, deductRate: 3 }
      : null
  );
  ...
  const [plUsed, setPlUsed] = useState(12.5);
  const plAvail = plLimit - plUsed;
  const plHistory = [
    { date: "Mar 7", amount: 12.5, route: "Akosombo → Atimpoku", status: "due" },
    { date: "Feb 28", amount: 9.0, route: "Kpong → Asesewa", status: "paid" },
    { date: "Feb 20", amount: 7.5, route: "Odumase → Somanya", status: "paid" },
  ];
        ...
        <div style={{ display: "flex", gap: 2 }}>
          {tabs.map((tb) => (
            <button
              ...
              style={{
                flex: 1, padding: "8px 2px", fontSize: 10, fontWeight: 700, borderRadius: "10px 10px 0 0",
          ...
          <>
            <div style={{ background: "linear-gradient(135deg,#4f46e5,#7c3aed)", borderRadius: 20, padding: "20px", color: "#fff" }}>
              ...
              <div style={{ display: "flex", gap: 20, marginTop: 8 }}>
                <div>
                  <p style={{ fontSize: 10, color: "#c7d2fe" }}>Interest Earned</p>
                  <p style={{ fontWeight: 700, fontSize: 14 }}>GH₵{savInt.toFixed(2)}</p>
                </div>
                <div>
                  <p style={{ fontSize: 10, color: "#c7d2fe" }}>Monthly Est.</p>
                  <p style={{ fontWeight: 700, fontSize: 14 }}>+GH₵{monthlyEst}</p>
                </div>
                <div>
                  <p style={{ fontSize: 10, color: "#c7d2fe" }}>Rate p.a.</p>
                  <p style={{ fontWeight: 700, fontSize: 14 }}>8%</p>
                </div>
              </div>
            </div>
            ...
            <div className={`${t.card} rounded-2xl p-4 border ${t.bdr}`}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <div>
                  <p className={`font-black text-sm ${t.text}`}>⚙️ Auto-Save from Earnings</p>
                  <p className={`text-xs ${t.sub}`}>{autoRate}% of each payout saved automatically</p>
                </div>
                <span style={{ fontWeight: 900, color: "#7c3aed", fontSize: 22 }}>{autoRate}%</span>
              </div>
              <input
                type="range" min="0" max="30" value={autoRate}
                onChange={(e) => setAutoRate(Number(e.target.value))}
                style={{ width: "100%", accentColor: "#7c3aed" }}
              />
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, marginBottom: 10 }} className={t.sub}>
                <span>0%</span><span>10%</span><span>20%</span><span>30%</span>
              </div>
              <button
                onClick={async () => {
                  try { await api.setSavingsRate(user.id, autoRate); } catch (err) { console.warn("Error:", err); }
                  toast$(`Auto-save set to ${autoRate}% ✅`);
                }}
                style={{ width: "100%", padding: "10px", background: "#7c3aed", color: "#fff", borderRadius: 12, fontWeight: 700, fontSize: 13 }}
              >
                Save Setting
              </button>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <button
                onClick={() => setSavStep("deposit")}
                style={{
                  padding: "14px", background: "#7c3aed", color: "#fff", borderRadius: 14, fontWeight: 700,
              ...
              <button
                onClick={() => setSavStep("withdraw")}
                style={{
                  padding: "14px", border: "2px solid #7c3aed", color: "#7c3aed", borderRadius: 14, fontWeight: 700,
                  display: "flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 13,
                }}
              >
            ...
            {(savStep === "deposit" || savStep === "withdraw") && (
              <div className={`${t.card} rounded-2xl p-4 border-2 border-purple-400`}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  ...
                  <button onClick={() => { setSavStep("home"); setSavInput(""); }}>
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      else await api.withdrawSavings(user.id, amt);
            ...
            <div className={`${t.card} rounded-2xl border ${t.bdr} overflow-hidden`}>
              ...
              {savHistory.map((h, i) => (
                ...
                >
                  <div>
                    <p className={`font-bold text-sm ${t.text}`}>{h.date}</p>
                    <p className={`text-xs ${t.sub}`}>Saved GH₵{h.deposited} · Interest +GH₵{h.interest}</p>
                  </div>
            ...
            <div style={{ background: dark ? "#1e1b4b" : "#eef2ff", borderRadius: 14, padding: "12px 14px" }}>
              ...
              <p style={{ fontSize: 11, color: dark ? "#a5b4fc" : "#4338ca" }}>
                8% annual interest calculated monthly. Save for 3+ months to unlock loan eligibility. Your savings history
                determines your credit score. Funds always accessible.
              </p>
            </div>
          </>
        )}
          ...
          <>
            <div style={{ background: "linear-gradient(135deg,#0f172a,#1e293b)", borderRadius: 20, padding: "20px", color: "#fff" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  ...
                  <p
                    style={{
                      fontWeight: 900, fontSize: 44, margin: "4px 0",
                      color: creditScore >= 650 ? "#4ade80" : creditScore >= 500 ? "#facc15" : "#f87171",
                    }}
                  >
                    {creditScore}
                  </p>
              ...
              <div style={{ marginTop: 14, height: 6, borderRadius: 999, background: "#334155" }}>
                <div
                  style={{
                    ...
                    width: `${Math.min(((creditScore - 300) / 550) * 100, 100)}%`,
                    background: creditScore >= 650 ? "#4ade80" : creditScore >= 500 ? "#facc15" : "#f87171",
                  }}
                />
              </div>
            ...
            <div className={`${t.card} rounded-2xl p-4 border ${t.bdr}`}>
              ...
              {(role === "driver"
                ...
                <p key={i} style={{ fontSize: 12, marginBottom: 4, color: r.startsWith("✅") ? "#16a34a" : r.startsWith("⏳") ? "#ca8a04" : "#ef4444" }}>
                  {r}
                </p>
              ))}
            </div>
            ...
            {activeLoan && (
              <div className={`${t.card} rounded-2xl p-4 border-2 border-purple-400`}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                  <div>
                    <p className={`font-bold ${t.text}`}>GH₵{activeLoan.amount} — {activeLoan.purpose}</p>
                    <p className={`text-xs ${t.sub}`}>GH₵{activeLoan.monthly}/month interest</p>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <p style={{ color: "#7c3aed", fontWeight: 900, fontSize: 16 }}>GH₵{activeLoan.remaining}</p>
                    <p className={`text-xs ${t.sub}`}>remaining</p>
                  </div>
                </div>
            ...
            {!activeLoan && loanStep === "home" && loanEligible && (
              <div className={`${t.card} rounded-2xl p-4 border ${t.bdr}`}>
                ...
                >
                  ...
                  {["Vehicle Repair", "Fuel Stock", "Medical Emergency", "School Fees", "Business Expansion", "Drive to Own Down Payment", "Ride Fare (Pay Later)", "Other"].map(
                    (p) => (
                ...
                {loanAmount && loanPurpose && (
                  <div style={{ background: dark ? "#1e1b4b" : "#eef2ff", borderRadius: 12, padding: "10px 12px", marginBottom: 12 }}>
                    {[
                      ["Loan Amount", `GH₵${loanAmount}`, "#7c3aed"],
                      ["Monthly Interest (5%)", `GH₵${(parseFloat(loanAmount || 0) * 0.05).toFixed(2)}`, t.text],
                      ["Est. repayment per ride", "~3% of earnings", "#16a34a"],
                    ].map(([l, v, c]) => (
                ...
                <button
                  onClick={async () => {
                    ...
                    setTimeout(() => {
                      setActiveLoan({
                        amount: parseFloat(loanAmount), remaining: parseFloat(loanAmount), repaid: 0,
                        monthly: +(parseFloat(loanAmount) * 0.05).toFixed(2), purpose: loanPurpose, progress: 0, deductRate: 3,
                      });
                  ...
                  style={{
                    ...
                    background: !loanAmount || !loanPurpose ? "#9ca3af" : "#7c3aed",
                    color: "#fff", borderRadius: 14, fontWeight: 900,
                  }}
                >
                  Apply for Loan →
                </button>
            ...
            {loanStep === "processing" && (
              <div style={{ textAlign: "center", padding: "32px 0" }}>
                <div
                  style={{
                    width: 52, height: 52, border: "4px solid #7c3aed", borderTopColor: "transparent",
            ...
            {loanStep === "approved" && (
              <div style={{ textAlign: "center", padding: "24px 0" }}>
                ...
                <button
                  onClick={() => setLoanStep("home")}
                  style={{ marginTop: 16, padding: "12px 28px", background: "#7c3aed", color: "#fff", borderRadius: 14, fontWeight: 900 }}
                >
                  Done ✅
                </button>
              </div>
        ...
        {tab === "insurance" && (
          <>
            <div style={{ background: "linear-gradient(135deg,#0369a1,#0284c7)", borderRadius: 20, padding: "20px", color: "#fff" }}>
              ...
              <p style={{ fontWeight: 900, fontSize: 26, margin: "4px 0" }}>
                {activePlan ? insPlans.find((p) => p.id === activePlan)?.name : "No Active Plan"}
              </p>
              <p style={{ fontSize: 12, color: "#bae6fd" }}>
                {activePlan
                  ? `Cover up to GH₵${insPlans.find((p) => p.id === activePlan)?.cover?.toLocaleString()}`
                  : "Select a plan below"}
              </p>
            </div>
            ...
            {insPlans.map((plan) => (
              <div
                key={plan.id}
                className={`${t.card} rounded-2xl p-4 border-2`}
                style={{ borderColor: activePlan === plan.id ? plan.color : dark ? "#374151" : "#e5e7eb" }}
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    <p style={{ fontWeight: 900, color: plan.color, fontSize: 18 }}>GH₵{plan.price}</p>
                    <p className={`text-xs ${t.sub}`}>/month</p>
                  </div>
                </div>
                <div
                  style={{
                    display: "flex", justifyContent: "space-between", padding: "8px 10px",
                    background: dark ? "#374151" : "#f9fafb", borderRadius: 10, marginBottom: 10,
                  }}
                >
                ...
                {activePlan !== plan.id ? (
                  ...
                  <button
                    onClick={() => setClaimStep("file")}
                    style={{ width: "100%", padding: "10px", border: `2px solid ${plan.color}`, color: plan.color, borderRadius: 12, fontWeight: 700, fontSize: 13 }}
                  >
                    📋 File a Claim
                  </button>
                )}
            ...
            {claimStep === "file" && (
              <div className={`${t.card} rounded-2xl p-4 border-2 border-blue-400`}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                  ...
                  <button onClick={() => setClaimStep("home")}>
                    <X style={{ width: 18, height: 18, color: "#9ca3af" }} />
                  </button>
                </div>
                  ...
                  {["Accident / Injury", "Vehicle Damage", "Third Party Damage", "Theft", "Medical Expenses", "Other"].map((ct) => (
                    <button
                      ...
                      style={{
                        ...
                        border: `2px solid ${claimType === ct ? "#0284c7" : dark ? "#374151" : "#e5e7eb"}`,
                        background: claimType === ct ? "#eff6ff" : "transparent",
                        fontWeight: 600, fontSize: 13, textAlign: "left",
                        color: claimType === ct ? "#0284c7" : dark ? "#d1d5db" : "#374151",
                      }}
                    >
                      {ct}
                    </button>
                ...
                <button
                  onClick={async () => {
                    ...
                    setClaimStep("done");
                    toast$("Claim submitted! Review within 24hrs ✅");
                  }}
                  style={{ width: "100%", padding: "12px", background: "#0284c7", color: "#fff", borderRadius: 14, fontWeight: 900 }}
                >
                  Submit Claim
                </button>
              </div>
            ...
            {claimStep === "done" && (
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                ...
                <button
                  onClick={() => setClaimStep("home")}
                  style={{ marginTop: 14, padding: "10px 24px", background: "#0284c7", color: "#fff", borderRadius: 12, fontWeight: 700 }}
                >
                  Done
                </button>
              </div>
            ...
            <div style={{ background: dark ? "#0c1a2e" : "#f0f9ff", borderRadius: 14, padding: "12px 14px" }}>
              ...
              <p style={{ fontSize: 11, color: dark ? "#7dd3fc" : "#0369a1" }}>
                Insurance underwritten by licensed partners. Okada Online is the distribution agent. All claims processed by
                the underwriting partner within 5 business days.
              </p>
            </div>
          </>
        )}
        ...
        {tab === "paylater" && (
          <>
            ...
            {plUsed > 0 && (
              <div
                style={{
                  background: dark ? "#1c1917" : "#fef3c7", borderRadius: 14, padding: "12px 14px",
              ...
              >
                ...
                <div>
                  ...
                  <p style={{ fontSize: 11, color: "#92400e", marginTop: 2 }}>
                    Due Mar 14. Auto-charged to MoMo. Missing payment permanently suspends Pay Later.
                  </p>
                  <button
                    onClick={async () => {
            ...
            <div className={`${t.card} rounded-2xl p-4 border ${t.bdr}`}>
              ...
              {[
                ["✅", "Book ride → choose Pay Later at checkout"],
                ["✅", "Ride now — payment deferred up to 7 days"],
                ["✅", "Auto-charged to your MoMo on due date"],
                ["✅", "Build credit history with on-time payments"],
                ["⚠️", "Missed payment permanently removes access"],
              ].map(([icon, text]) => (
                <p key={text} style={{ fontSize: 12, marginBottom: 5, color: icon === "⚠️" ? "#ef4444" : dark ? "#d1d5db" : "#374151" }}>
                  {icon} {text}
                </p>
              ))}
            </div>
            ...
            <div className={`${t.card} rounded-2xl border ${t.bdr} overflow-hidden`}>
              ...
              {plHistory.map((h, i) => (
                ...
                >
                  <div>
                    <p className={`font-bold text-sm ${t.text}`}>{h.route}</p>
                    <p className={`text-xs ${t.sub}`}>{h.date}</p>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <p style={{ fontWeight: 900, color: "#16a34a" }}>GH₵{h.amount}</p>
                    <Badge color={h.status === "paid" ? "green" : "yellow"}>{h.status}</Badge>
                  </div>
                </div>
              ))}
            </div>
