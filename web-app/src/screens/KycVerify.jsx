...
export function KycVerify({ role, onVerified, dark }) {
  ...
  const handleDocPhoto = (e) => {
    const f = e.target.files[0];
    if (!f) return;
    setDocPhoto(f);
    setDocPhotoP(URL.createObjectURL(f));
    setStep("selfie");
  };
  ...
  const handleSelfie = (e) => {
    ...
    if (!f) return;
    setSelfie(f);
    setSelfieP(URL.createObjectURL(f));
    setStep("review");
  };
    ...
    setTimeout(() => {
      ...
      setTimeout(
        () =>
          onVerified({
            type: docType,
            docNumber: isIntl ? passNum : cardNum,
            country: isIntl ? country : "Ghana",
            verified: true,
          }),
        1500
      );
  ...
  return (
    <div className={`min-h-screen ${t.bg} flex flex-col`}>
      ...
      <div
        style={{
          background: `linear-gradient(135deg,#1d4ed8,${isIntl ? "#7c3aed" : "#2563eb"})`,
          color: "#fff",
      ...
      >
        ...
        <h1 style={{ fontFamily: "Syne,sans-serif", fontWeight: 900, fontSize: 22 }}>
        ...
        <p style={{ color: "#c7d2fe", fontSize: 11, marginTop: 2 }}>
          Required for all {role}s · Enables Pay Later & Fintech
        </p>
          ...
          {steps.map((s, i) => (
            <div
              ...
              style={{
                ...
                height: 8,
                borderRadius: "50%",
                background: steps.indexOf(step) >= i ? "#fff" : "rgba(255,255,255,0.3)",
              }}
            />
          ))}
        </div>
      ...
      <div style={{ flex: 1, padding: "20px 16px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <>
            <div className={`${t.card} rounded-2xl p-5 border ${t.bdr}`}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <button
                  onClick={() => { setDocType("ghana"); setStep("details"); }}
                  style={{
                    display: "flex", alignItems: "center", gap: 14, padding: "16px",
                    borderRadius: 16, border: "2px solid #2563eb", background: "#eff6ff", textAlign: "left",
                  }}
                >
                ...
                <button
                  onClick={() => { setDocType("passport"); setStep("details"); }}
                  style={{
                    display: "flex", alignItems: "center", gap: 14, padding: "16px",
                    borderRadius: 16, border: "2px solid #7c3aed", background: "#f5f3ff", textAlign: "left",
                  }}
                >
                  <span style={{ fontSize: 32 }}>🛂</span>
                  <div style={{ flex: 1 }}>
            ...
            <div className={`${t.card} rounded-2xl p-4 border ${t.bdr}`}>
              ...
              {[
                "Safety for all passengers & drivers",
                "Required for Pay Later & fintech services",
                "NIA Ghana Card — instant data, no typing errors",
                "Passport accepted from 190+ countries",
                "Ghana Data Protection Act 2012 compliant",
                "International visitors — welcome to Ghana! 🇬🇭",
              ].map((r) => (
                <p key={r} className={`text-xs ${t.sub}`} style={{ marginBottom: 3 }}>
                  ✓ {r}
                </p>
              ))}
            </div>
        ...
        {step === "details" && (
          <div className={`${t.card} rounded-2xl p-5 border ${t.bdr}`}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              <div
                style={{
                  width: 36, height: 36, borderRadius: "50%", background: accentBg,
              ...
              <div>
                ...
                <p className={`text-xs ${t.sub}`}>
                  {isIntl ? "Country + passport number" : "Format: GHA-XXXXXXXXX-X"}
                </p>
              </div>
            </div>
            {isIntl && (
              <>
                ...
                <input
                  value={passNum}
                  onChange={(e) => setPassNum(e.target.value.toUpperCase())}
            ...
            {!isIntl && (
              <input
                value={cardNum}
                onChange={(e) => setCardNum(fmtGhana(e.target.value))}
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <button
                ...
                style={{
                  flex: 1, padding: "12px", border: `1px solid ${dark ? "#374151" : "#e5e7eb"}`,
              ...
              <button
                ...
                style={{
                  flex: 2, padding: "12px", background: accentColor, color: "#fff",
                  borderRadius: 14, fontWeight: 900, fontSize: 13,
                }}
              >
                Next — Upload Photo →
              </button>
          ...
          <div className={`${t.card} rounded-2xl p-5 border ${t.bdr}`}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              ...
              <div>
                ...
                <p className={`text-xs ${t.sub}`}>
                  {isIntl ? "Data page — all text visible" : "Front side — clear & well-lit"}
                </p>
              </div>
            </div>
            {docPhotoP ? (
              <div style={{ position: "relative", marginBottom: 12 }}>
                ...
                />
                <button
                  onClick={() => { setDocPhotoP(null); setDocPhoto(null); }}
                  style={{
              ...
              <label
                style={{
                  display: "block", border: `2px dashed ${accentColor}`, borderRadius: 16,
                  padding: "32px 16px", textAlign: "center", cursor: "pointer",
                  background: accentBg, marginBottom: 12,
                }}
              >
          ...
          <div className={`${t.card} rounded-2xl p-5 border ${t.bdr}`}>
            ...
            {selfieP ? (
              <div style={{ position: "relative", marginBottom: 12 }}>
                ...
                />
                <button
                  onClick={() => { setSelfieP(null); setSelfie(null); }}
                  style={{
                    position: "absolute", top: 8, right: 8, background: "#dc2626", color: "#fff",
                    borderRadius: "50%", width: 28, height: 28, display: "flex",
                    alignItems: "center", justifyContent: "center",
              ...
              <label
                style={{
                  display: "block", border: "2px dashed #86efac", borderRadius: 16,
                  padding: "32px 16px", textAlign: "center", cursor: "pointer",
                  background: dark ? "#14532d" : "#f0fdf4", marginBottom: 12,
                }}
              >
                <div style={{ fontSize: 36, marginBottom: 8 }}>🤳</div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              {selfieP && (
                <button
                  ...
                  style={{
                    flex: 2, padding: "12px", background: "#16a34a", color: "#fff",
                    borderRadius: 14, fontWeight: 900, fontSize: 13,
                  }}
                >
                  Review →
        ...
        {step === "review" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div className={`${t.card} rounded-2xl p-5 border ${t.bdr}`}>
              ...
              <div
                style={{
                  ...
                  background: dark ? "#374151" : "#f9fafb", marginBottom: 10,
                }}
              >
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <span className={`text-xs ${t.sub}`}>Document Type</span>
                  <span style={{ fontWeight: 700, fontSize: 12 }} className={t.text}>
                    {isIntl ? "International Passport 🛂" : "Ghana Card 🪪"}
                  </span>
                </div>
                {isIntl && (
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  ...
                  <span style={{ fontFamily: "monospace", fontWeight: 900, color: accentColor, fontSize: 12 }}>
                    {isIntl ? passNum : cardNum}
                  </span>
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {docPhotoP && (
                  <div style={{ textAlign: "center" }}>
                    <p className={`text-xs ${t.sub} mb-1`}>{isIntl ? "Passport" : "Card"}</p>
                    <img
                      src={docPhotoP}
                      alt="doc"
            ...
            <div
              style={{
                background: dark ? "#1e3a5f" : "#eff6ff", borderRadius: 14,
                padding: "12px 14px", border: "1px solid #bfdbfe",
              }}
            >
              <p style={{ color: "#2563eb", fontWeight: 700, fontSize: 12, marginBottom: 4 }}>🔒 Privacy Notice</p>
              <p style={{ fontSize: 11, color: dark ? "#93c5fd" : "#1e40af" }}>
                Your ID data is encrypted and used solely for identity verification. Never sold. Protected
                under Ghana Data Protection Act 2012 and GDPR for international users.
              </p>
            </div>
            <button
              onClick={submit}
              style={{
                ...
                borderRadius: 16, fontWeight: 900, fontSize: 15, display: "flex",
        ...
        {step === "processing" && (
          <div style={{ textAlign: "center", padding: "48px 0" }}>
            <div
              style={{
                width: 64, height: 64, border: `4px solid ${accentColor}`,
                borderTopColor: "transparent", borderRadius: "50%",
                animation: "spin 1s linear infinite", margin: "0 auto 20px",
              }}
            />
            <p className={`font-black text-lg ${t.text}`}>Verifying Identity…</p>
            <p className={`text-xs mt-2 ${t.sub}`}>
              {isIntl ? "Checking international document database" : "Checking with NIA Ghana"}
            </p>
          </div>
        )}
