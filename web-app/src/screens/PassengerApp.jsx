...
export function PassengerApp({ user, onLogout, dark, setDark }) {
  ...
  useEffect(() => {
    ...
    setFare({
      km: baseKm, total: totalFare, dur, surge,
      breakdown: { base: parseFloat(baseFare.toFixed(2)), surgeLabel: surge > 1 ? (surge === 1.5 ? "Late Night" : "Rush Hour") : "Normal" },
    });
  }, [pickup, dest, vehicle]);
      ...
      api.getHistory(user.id).then((r) => setHistory(r.rides || [])).catch(() =>
        setHistory(
          Array.from({ length: 6 }, (_, i) => ({
            ...
            fare: (Math.random() * 20 + 5).toFixed(2), date: new Date(Date.now() - i * 86400000 * 2).toLocaleDateString(),
            driver: "Kwame A.", rating: 5, vehicle: VEHICLES[i % 4].label,
            paid: ["MoMo", "Cash", "Card", "Pay Later"][i % 4],
          }))
        )
      );
    }
  ...
  const bookRide = async () => {
    ...
    try {
      await api.requestRide({
        ...
        destination: { address: dest, latitude: 6.15, longitude: 0.15 },
        rideType: vehicle,
      });
    ...
    setTimeout(() => {
      ...
      setStatus("matched");
      setEta(180);
      toast$(`${v.label} driver matched! ${v.icon}`);
    }, 3500);
    setLoading(false);
  };
  ...
  return (
    <div className={`max-w-md mx-auto min-h-screen relative ${t.bg}`}>
      ...
      <div style={{ background: "#16a34a", color: "#fff", padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <button onClick={() => setDark(!dark)} style={{ padding: 6, borderRadius: 8, background: "rgba(255,255,255,0.15)" }}>
            {dark ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
          </button>
          <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#15803d", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>
            {user.profilePhoto || "👤"}
          </div>
        </div>
      </div>
              ...
              {status === "idle" && (
                <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8 }}>
                    {VEHICLES.map((v) => (
                      <button key={v.id} onClick={() => setVehicle(v.id)} style={{ padding: "10px 4px", borderRadius: 12, border: `2px solid ${vehicle === v.id ? "#16a34a" : "#e5e7eb"}`, background: vehicle === v.id ? "#f0fdf4" : "transparent", textAlign: "center" }}>
            ...
            <div className={`${t.card} rounded-t-3xl p-6 w-full shadow-2xl`} style={{ maxHeight: "90vh", overflowY: "auto" }}>
              ...
              {["mtn", "vodafone", "airtel"].includes(payMethod) && (
                <div style={{ marginBottom: 12 }}>
                  ...
                  <input value={momoPhone} onChange={(e) => setMomoPhone(e.target.value)} className={`w-full px-4 py-3 border rounded-xl text-sm focus:outline-none ${t.inp}`} style={{ display: "block", width: "100%" }} placeholder="+233XXXXXXXXX" />
                </div>
              )}
              {payMethod === "card" && (
                <div style={{ marginBottom: 12 }}>
                  <p className={`text-xs font-bold mb-1 ${t.sub}`}>EMAIL</p>
                  <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" className={`w-full px-4 py-3 border rounded-xl text-sm focus:outline-none ${t.inp}`} style={{ display: "block", width: "100%" }} placeholder="your@email.com" />
                </div>
              )}
              {payMethod === "cash" && (
              ...
              <button disabled={!payMethod} onClick={async () => {
                ...
                if (payMethod === "cash") {
                  ...
                  setTimeout(() => {
                    ...
                    toast$("Cash confirmed by driver ✅");
                    setTimeout(() => setPayStatus("idle"), 2500);
                  }, 4000);
                  return;
                }
                if (payMethod === "paylater") {
                  ...
                  toast$(`GH₵${fare.total} deferred — Pay Later used ⏳`);
                ...
                setTimeout(() => {
                  setPayStatus("paid"); setStatus("idle"); setDriver(null); setPickup(""); setDest(""); setFare(null);
                  toast$(`GH₵${fare.total} paid via ${payMethod.toUpperCase()} ✅`);
                  setTimeout(() => setPayStatus("idle"), 3000);
                }, 2500);
