...
export function WithdrawSheet({ available, pending, userId, onClose, dark }) {
  ...
  const withdraw = async () => {
    ...
    setStep("processing");
    try {
      await api.requestWithdrawal(userId, parseFloat(amount), momoPhone);
    } catch (err) {
  ...
  return (
    <div
      style={{
        ...
        display: "flex", alignItems: "flex-end", maxWidth: 448, margin: "0 auto",
      }}
    >
      ...
      <div className={`${t.card} rounded-t-3xl p-6 w-full shadow-2xl`} style={{ maxHeight: "90vh", overflowY: "auto" }}>
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
              {[25, 50, 100, "All"].map((v) => (
                <button
                  key={v}
                  onClick={() => setAmount(v === "All" ? available.toFixed(2) : String(Math.min(v, available)))}
                  style={{
                    flex: 1, padding: "6px", borderRadius: 8,
                    border: `1px solid ${dark ? "#374151" : "#e5e7eb"}`, fontSize: 12, fontWeight: 700,
                  }}
                  className={t.text}
                >
                  {v === "All" ? "All" : "+" + v}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginBottom: 12 }}>
              {[["mtn", "MTN", "#ffcc00", "#000"], ["vodafone", "Vodafone", "#e60000", "#fff"], ["airtel", "AirtelTigo", "#ef4444", "#fff"]].map(
                ([id, lb, bg, fg]) => (
                  <button
                    key={id}
                    ...
                    style={{
                      ...
                      border: `2px solid ${network === id ? bg : "#e5e7eb"}`,
                      background: network === id ? bg : "transparent",
                      fontWeight: 700, fontSize: 11,
                      color: network === id ? fg : (dark ? "#9ca3af" : "#6b7280"),
                    }}
                  >
                    {lb}
                  </button>
            ...
            <button
              onClick={withdraw}
              disabled={!amount || !momoPhone || parseFloat(amount) > available}
              style={{
                width: "100%", padding: "14px",
                background: (!amount || !momoPhone || parseFloat(amount) > available) ? "#9ca3af" : "#16a34a",
                color: "#fff", borderRadius: 16, fontWeight: 900,
        ...
        {step === "processing" && (
          <div style={{ textAlign: "center", padding: "32px 0" }}>
            <div
              style={{
                width: 52, height: 52, border: "4px solid #16a34a", borderTopColor: "transparent",
        ...
        {step === "done" && (
          <div style={{ textAlign: "center", padding: "32px 0" }}>
            ...
            <button
              onClick={onClose}
              style={{ marginTop: 20, padding: "12px 32px", background: "#16a34a", color: "#fff", borderRadius: 14, fontWeight: 900 }}
            >
              Done
            </button>
          </div>
