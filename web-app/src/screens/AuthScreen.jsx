...
export function AuthScreen({ onLogin, dark, apiStatus = "checking" }) {
  ...
  const sendOtp = async () => {
    ...
    if (!/^\+233[0-9]{9}$/.test(cleaned)) {
      toast$("Enter valid Ghana number: +233XXXXXXXXX", "error");
      return;
    }
  ...
  return (
    <div className={`min-h-screen flex flex-col ${t.bg}`}>
      ...
      >
        ...
        <h1 style={{ fontFamily: "Syne,sans-serif", fontSize: 36, fontWeight: 900, letterSpacing: "-0.02em" }}>
          Okada Online
        </h1>
        <p style={{ marginTop: 6, color: "#bbf7d0", fontSize: 14, fontWeight: 600 }}>
          Eastern Region Ghana · Complete Transport & Fintech Ecosystem 🇬🇭
        </p>
        <div
          style={{
            marginTop: 8, display: "inline-flex", alignItems: "center", gap: 6,
            background: "rgba(0,0,0,0.2)", borderRadius: 999, padding: "4px 12px",
          }}
        >
          <div
            style={{
        ...
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 8, marginTop: 14 }}>
          {["Owner 50%", "Driver 25%", "Fuel 5%", "Maint. 5%", "Savings 8%", "Loans", "Insurance", "Pay Later", "🌍 Intl"].map((f) => (
            <span key={f} style={{ background: "rgba(255,255,255,0.15)", borderRadius: 999, padding: "4px 10px", fontSize: 11, fontWeight: 700 }}>
              {f}
            </span>
          ))}
        </div>
      ...
      <div style={{ flex: 1, padding: "24px 20px" }}>
        <div
          style={{
            display: "grid", gridTemplateColumns: "repeat(4,1fr)", borderRadius: 16, overflow: "hidden",
            border: `1px solid ${dark ? "#374151" : "#e5e7eb"}`, marginBottom: 20,
          }}
        >
          {[["passenger", "🧍", "Passenger"], ["driver", "🏍️", "Driver"], ["owner", "🏢", "Owner"], ["admin", "⚙️", "Admin"]].map(
            ([r, ic, lb]) => (
              <button
                key={r}
                onClick={() => { setRole(r); setStep("phone"); setKycData(null); }}
                style={{
                  padding: "10px 4px", fontSize: 10, fontWeight: 700,
                  background: role === r ? "#16a34a" : "transparent",
                  color: role === r ? "#fff" : dark ? "#9ca3af" : "#6b7280",
                  textAlign: "center",
                }}
              >
        ...
        {step === "phone" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {role === "driver" && (
              <div>
                <input
                  ...
                  placeholder="Owner Code (e.g. OWNXYZ123)"
            ...
            {role !== "admin" && (
              <div
                className={`rounded-2xl p-4 border-2 ${kycData ? "border-green-500" : "border-blue-400"}`}
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <div>
                      ...
                      <p className={`text-xs ${t.sub}`}>
                        {kycData
                          ? `${kycData.type === "ghana" ? "Ghana Card" : "Passport"}: ${kycData.docNumber}`
                  ...
                  {kycData ? (
                    ...
                    <button
                      onClick={() => setStep("kyc")}
                      style={{ padding: "6px 12px", background: "#2563eb", color: "#fff", borderRadius: 10, fontWeight: 700, fontSize: 12 }}
                    >
                      Verify →
                    </button>
                  )}
                ...
                {!kycData && (
                  <p className={`text-xs mt-2 ${t.sub}`}>
                    🌍 International visitors welcome — use your passport · Required for fintech & Pay Later
                  </p>
                )}
              </div>
            )}
            <button
              ...
              style={{
                width: "100%", background: otpCooldown > 0 ? "#9ca3af" : "#16a34a", color: "#fff",
                padding: "14px", borderRadius: 16, fontWeight: 900, fontSize: 16,
                display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
                opacity: loading || otpCooldown > 0 ? 0.7 : 1,
              }}
            >
              {loading && <Spin />}
        ...
        {step === "otp" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <button
              ...
              disabled={loading}
              style={{
                width: "100%", background: "#16a34a", color: "#fff", padding: "14px", borderRadius: 16,
        ...
        <div className="grid grid-cols-2 gap-3 mt-6">
          {[["🪪", "Ghana Card KYC"], ["🌍", "Intl Passport"], ["🏦", "Savings 8% p.a."], ["💳", "Earnings Loans"], ["🛡️", "Insurance"], ["⏳", "Pay Later"]].map(
            ([i, l]) => (
