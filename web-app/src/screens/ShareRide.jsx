...
export function ShareRide({user,dark,onBack}) {
  ...
  const vIcon={car:"🚗",okada:"🏍️",tricycle:"🛺",ev_car:"⚡🚗"};
    ...
    <div style={{minHeight:"100%"}} className={t.bg}>
      ...
      <div style={{background:"linear-gradient(135deg,#0d9488,#0284c7)",color:"#fff",padding:"14px 16px",display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:20}}>
        ...
        <div>
          ...
          <p style={{fontSize:11,margin:0,opacity:0.8}}>Split the fare · Same route · Save money</p>
        </div>
      ...
      <div style={{padding:16,display:"flex",flexDirection:"column",gap:14}}>
            ...
            {trips.map(trip=>(
              <div key={trip.id} className={t.card+" rounded-2xl p-4 border-2"} style={{borderColor:trip.status==="full"?"#9ca3af":"#0d9488"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                  <div>
                    <p className={"font-black "+t.text}>{vIcon[trip.vehicleType]||"🚗"} {trip.pickupAddress} → {trip.destAddress}</p>
                    <p className={"text-xs "+t.sub}>⏰ {trip.departTime} · {trip.departDate}</p>
        ...
        {view==="create"&&(
          <>
            ...
            <div className={t.card+" rounded-2xl p-4 border "+t.bdr}>
              <p className={"font-black mb-3 "+t.text}>Vehicle</p>
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:4}}>
                {[{id:"car",icon:"🚗"},{id:"okada",icon:"🏍️"},{id:"tricycle",icon:"🛺"},{id:"ev_car",icon:"⚡🚗"}].map(v=>(
                  <button key={v.id} onClick={()=>setVehicle(v.id)}
                    style={{padding:"10px 4px",borderRadius:12,textAlign:"center",border:`2px solid ${vehicle===v.id?"#0d9488":"#e5e7eb"}`,background:vehicle===v.id?"#f0fdfa":"transparent",cursor:"pointer"}}>
