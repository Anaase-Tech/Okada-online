...
import { MapPin, Navigation, AlertCircle, LogOut, ArrowDownCircle, Clock } from "lucide-react";
import { api } from "../api";
import { T } from "../theme";
          ...
          (snap) => {
            if (!snap.empty) {
              ...
              setIncoming({
                ...
                earn: "GH₵" + ((r.fare || r.total || 0) * 0.1).toFixed(2),
            ...
            const tm = setTimeout(
              () =>
                setIncoming({
                  ...
                  fare: "GH₵13.50", earn: "GH₵1.35",
                  payMethod: ["mtn", "cash", "vodafone"][Math.floor(Math.random() * 3)],
                }),
              5000
            );
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center", maxWidth: 448, margin: "0 auto", padding: "0 20px" }}>
          <div className={`${t.card} rounded-3xl p-6 w-full shadow-2xl`}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <button onClick={() => { setCashConfirm(null); toast$("Dispute raised — admin will review", "error"); }} style={{ padding: "12px", border: "1px solid #f87171", color: "#ef4444", borderRadius: 14, fontWeight: 700, fontSize: 13 }}>❌ Not Received</button>
        ...
        {view === "home" && (
          <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div className={`${t.card} rounded-2xl p-4 border-2 border-yellow-500`}>
              ...
              <button onClick={() => setShowFuelCode(!showFuelCode)} style={{ width: "100%", padding: "10px", background: "#ca8a04", color: "#fff", borderRadius: 12, fontWeight: 900, fontSize: 13 }}>
